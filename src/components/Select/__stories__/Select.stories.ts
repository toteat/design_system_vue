...
export const InsideScrollContainer: Story = {
  render: () => ({
    components: { Select },
    setup() {
      const withAppendValue = ref<string | number | null>(null);
      const withoutAppendValue = ref<string | number | null>(null);
      return { withAppendValue, withoutAppendValue, fruitOptions };
    },
    template: `
      <div style="max-width: 800px;">
        <h3 style="margin-bottom: 1rem;">Select Inside Overflow Container</h3>
        <p style="margin-bottom: 1.5rem; color: #666; font-size: 0.875rem;">
          When a Select is inside a container with <code>overflow: auto/scroll/hidden</code>,
        ...
        <div style="margin-top: 2rem; padding: 1rem; background-color: #f8f9fa; border-radius: 0.5rem;">
          <strong>Usage:</strong>
          <pre style="margin: 0.5rem 0 0 0; background-color: #e9ecef; padding: 0.75rem; border-radius: 0.25rem; overflow-x: auto;">&lt;Select :options="options" v-model="value" append-to-body /&gt;</pre>
        </div>
